...
export default {
    ...
    reducers: {
        ...
        deleteSuccess(state,  { payload }) {
            console.log("sale-reducer - deleteSuccess");
            console.log("payload:"+ payload);
            let sale = state.list;
            for (let i = 0; i < payload.length; i ++) {
                sale = sale.filter(s => s.id != payload[i]);
            }
            return { ...state, list: sale };
        },
        ...
        updateSuccess(state, { payload }) {
            const newList = state.list.map(sale => {
